...
contract SnowMerkle is Script, ScriptHelper {
    using stdJson for string;

    Merkle private m = new Merkle();

    string private inputPath = "/script/flakes/input.json";
    string private outputPath = "/script/flakes/output.json";
    ...
    // make five arrays the same size as the number of leaf nodes
    ...
    function run() public {
        ...
        for (uint256 i = 0; i < count; ++i) {
            ...
            for (uint256 j = 0; j < types.length; ++j) {
                ...
                } else if (compareStrings(types[j], "uint256")) {
